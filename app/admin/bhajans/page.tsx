...
import { Plus, Edit, Trash2, Sparkles, X, Check, Music } from 'lucide-react';
...
export default function ManageBhajansPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/bhajans/${id}`, { method: 'DELETE' });
      loadData();
    } catch (err) {
  ...
  return (
    <div className="space-y-8 font-gujarati">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-maroon-950">ભજનોનું સંચાલન (Manage Bhajans)</h1>
          <p className="text-maroon-800/70 text-sm mt-1">નવા ભજન ઉમેરો, એડિટ કરો કે સ્ટેટસ બદલો.</p>
        </div>
          ...
          <tbody className="divide-y divide-cream-200">
            {bhajans.map((b) => (
              <tr key={b.id} className="hover:bg-cream-100/50 transition">
                ...
                <td className="p-4">{b.audioUrl ? <Music className="w-4 h-4 text-saffron-600" /> : '-'}</td>
                <td className="p-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-sm">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-3 rounded-xl border border-saffron-500/30 bg-white leading-relaxed"
                  placeholder="ભજનના પૂરા શબ્દો લખો..."
                />
              </div>
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-saffron-500/20">
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 rounded-xl bg-saffron-600 text-cream-50 font-bold hover:bg-saffron-700 shadow"
                >
                  સેવ કરો
                </button>
              </div>
